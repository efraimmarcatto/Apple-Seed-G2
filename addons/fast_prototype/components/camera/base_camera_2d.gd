# BaseCamera2D.gd
# Classe base para todas as câmeras do jogo. Contém efeitos e utilitários reutilizáveis:
# - Screen shake (tremor)
# - Dynamic zoom (zoom suave)
# - Focus shift (mover camera para um ponto)
# - Transition to another camera (interpolação completa de posição/zoom/rotation)
# Sinais informam começo/fim de cada efeito para que outros sistemas (UI, SFX) reajam.
...
signal shake_started      ## Emitted when a screen shake begins.
signal shake_ended        ## Emitted when the screen shake finishes.
signal zoom_started       ## Emitted when a zoom transition begins.
signal zoom_ended         ## Emitted when the zoom transition completes.
signal focus_started      ## Emitted when a focus_on_point begins (camera moving to a given point).
signal focus_ended        ## Emitted when the focus finishes.
signal transition_started ## Emitted when a transition to another camera starts.
signal transition_ended   ## Emitted when the transition completes (and target camera is set to current).
