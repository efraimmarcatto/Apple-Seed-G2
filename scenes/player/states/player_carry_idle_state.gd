...
func _on_state_enter(_last_state_name:String) -> void:
	if animation_tree:
		if character_movement_controller:
			animation_tree.set("parameters/carry_idle/BlendSpace2D/blend_position", character_movement_controller.last_movement_direction.normalized())
		animation_tree["parameters/playback"].travel("carry_idle")
