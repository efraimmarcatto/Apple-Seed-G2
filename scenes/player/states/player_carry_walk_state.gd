...
func _on_state_next_transitions() -> void:
	if character_movement_controller:
		if not player.is_carrying() and character_movement_controller.is_able_to_walk():
			transition_to("walk")
		elif player.is_carrying() and not character_movement_controller.is_able_to_walk():
