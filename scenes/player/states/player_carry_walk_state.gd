...
func _on_state_physics_process(delta : float) -> void:
	if player and character_movement_controller:
		...
		if animation_tree and character_movement_controller.movement_direction != Vector2.ZERO:
			animation_tree.set("parameters/carry_walk/BlendSpace2D/blend_position", character_movement_controller.movement_direction.normalized())
...
func _on_state_next_transitions() -> void:
	if player and player.carry_controller and character_movement_controller:
		if not player.carry_controller.is_carrying() and character_movement_controller.is_able_to_walk():
			transition_to("walk")
		elif player.carry_controller.is_carrying() and not character_movement_controller.is_able_to_walk():
			transition_to("carry_idle")
	
# Função chamada ao entrar neste estado
func _on_state_enter(_last_state_name:String) -> void:
	if animation_tree:
		animation_tree["parameters/playback"].travel("carry_walk")
